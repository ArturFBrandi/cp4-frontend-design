...
### 2. Página de Categorias — `src/categorias.html`
...
- Aceita a categoria pela URL: `src/categorias.html?cat=casa`.

### 3. Página de Detalhes do Produto — `src/produto.html`
- Imagem em destaque, nome, marca, preço (com preço anterior), descrição completa,
  tabela de especificações e botão de compra.

### 4. Página de Contato — `src/contato.html`
...
| **Estilização personalizada** | Sobrescrita das variáveis do Bootstrap (`--bs-*`) e de seus componentes em `src/css/style.css`, com paleta, tipografia, botões, cards e formulários próprios da marca. |
